...
editing bounds in multiple places. For a temporal GridSpec, `steps` is the
number of timesteps and `steps_high` is the inclusive upper bound. Use
`{{ steps_high }}` when rendering `steps.high`; the temporal fields are also
available as `time_type`, `time_start`, `time_count`, and either
`time_unit`/`time_increment` or `time_interval`.
