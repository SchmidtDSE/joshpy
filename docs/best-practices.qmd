...
The registry stores experiment data across eight related tables. The `run_hash` is the
...
| `run_tags` | Free-form JSON metadata, keyed by scope | scope, key, tags |
...
created_at: TIMESTAMP
```
When the same config is run across multiple sessions (pooling replicates),
each session gets a row here. This is what `get_configs_for_session()` queries.
...
metadata: JSON
```
Each row represents one CLI invocation. `session_id` records which session
created this run, so session summaries count only their own runs.
...
`run_id` ties each row to a specific CLI invocation — critical for distinguishing
pooled replicates that share the same replicate number.
...
**run_tags**
```
scope: VARCHAR (composite primary key with key)
key: VARCHAR (composite primary key with scope)
tags: JSON
updated_at: TIMESTAMP
```
Deliberately separate from `config_parameters` -- setting a tag never `ALTER`s
another table's schema. `scope` names what kind of thing `key` is:
`"run_hash"`, `"session_id"`, and `"run_id"` are validated (the target must
already exist), plus any custom scope name for groupings that don't
correspond to a column anywhere (e.g. a site code shared by many run_hashes).
See [Tagging Runs with Free-Form Metadata](#tagging-runs-with-free-form-metadata).
:::
...
### Tagging Runs with Free-Form Metadata

`config_parameters` holds sweep parameters declared up front, one typed column
per parameter, one value per run_hash. Sometimes you need to attach metadata
that wasn't a sweep parameter at all -- and doing that by `ALTER TABLE`-ing
`config_parameters` ad hoc risks type collisions across unrelated tags and
blurs "what I swept" with "what I annotated afterward". The `run_tags` table
is a separate, additive-only sidecar for exactly this.

Run-level factors -- what a run is *about* (site, treatment, scenario) and the
join keys linking runs to Josh's CSV output -- are **attributes**, the
sanctioned home that replaces encoding factors into the label string:

```python
registry.set_attributes(run_hash, site="JOTR001", biome="desert")
registry.get_attributes(run_hash)
# {'site': 'JOTR001', 'biome': 'desert'}
```

`DiagnosticQueries.get_parameter_comparison(variable, param_name)` falls back
to attributes for any `param_name` that isn't a declared sweep parameter, so
`group_by="site"` works with no extra query code once set.

`find_by_attribute()` is the reverse lookup (attribute value → run_hashes) and
is **currency-aware**: it returns active runs only by default, so superseded or
`bad` runs never leak into an analysis pool (pass `current_only=False` to
include them). `find_current_by_attribute()` returns the same, resolved with
label and attributes.

```python
registry.find_by_attribute("site", "JOTR001")
# ['hash_jotr001', 'hash_jotr002']  # active only
```

Attributes are the `run_hash` slice of a broader `run_tags` sidecar. Two more
validated scopes carry general metadata (not run factors), each checking its
target exists and documented against exactly what it joins:

| Scope | Set with | Joins against |
|-------|----------|----------------|
| `run_hash` | `set_attributes(run_hash, **attributes)` | `job_configs.run_hash`, `config_parameters.run_hash`, `job_runs.run_hash`, `cell_data.run_hash` (same value, denormalized on purpose) |
| `session_id` | `tag_by_session_id(session_id, **tags)` | `sweep_sessions.session_id`, `job_runs.session_id`, `session_configs.session_id` |
| `run_id` | `tag_by_run_id(run_id, **tags)` | `job_runs.run_id`, `cell_data.run_id`, `run_outputs.run_id` (via `job_runs`) |

For metadata about a grouping that's no column in the schema -- the *site*
itself (plot count, elevation), not any one run -- use a custom scope, with
`find_tagged()` for its reverse lookup:

```python
registry.tag_custom("JOTR001", scope="site", n_plots=12, elevation_m=850)
registry.find_tagged("biome", "desert", scope="site")
# ['JOTR001', 'JOTR002', ...]
```

`tag_custom()` refuses the three validated scope names -- use `set_attributes`
(run_hash) or the matching `tag_by_*` method for those. See
[Tagging Runs for Custom Grouping](tutorials/project-organization.qmd#tagging-runs-for-custom-grouping)
...
- **Pooling replicates** — running the same config across multiple sessions to
  accumulate more replicates (e.g., 2 reps in session 1, then 3 more in session 2)

If you do this, **filter by `session_id`** in your queries. For runs, filter
directly on `job_runs.session_id`. For configs, join through `session_configs`:
...
### Sharing & Syncing via S3

DuckDB has no writable, concurrent-safe "live" database over S3 -- a registry
is always a local file. What's supported instead is publishing a registry's
tables to S3 as Parquet, and restoring or merging them back later, reusing
the same S3/MinIO credentials `ingest_results()` already resolves
(`MINIO_ENDPOINT`, `MINIO_ACCESS_KEY`, `MINIO_SECRET_KEY`, or explicit args).

```python
# Publish (defaults to run-registries/<db filename stem>/ on the bucket)
registry.push_to_s3(bucket="josh-batch-storage")

# Rehydrate this registry from its own prior export -- e.g. a new machine,
# or after deleting the local file. Existing rows are replaced with exactly
# what's published; table constraints (PKs, the cell_id sequence) survive.
registry.pull_from_s3(bucket="josh-batch-storage", mode="restore")

# Add only rows this registry doesn't already have -- for bringing in more
# replicates of the *same* experiment from a teammate's machine or a batch
# worker publishing to the same prefix. cell_data dedupes on
# (run_hash, replicate), not cell_id, which is a per-registry surrogate with
# no meaning across registries.
registry.pull_from_s3(bucket="josh-batch-storage", mode="merge")
```

**Don't use `mode="merge"` to combine genuinely different experiments into
one registry** -- see [Why Not Reuse Registries?](#why-not-reuse-registries)
above for exactly why that risks the same hash-collision ambiguity. To
analyze several published registries together without merging their data,
use `open_s3_registries()`, which opens each as a read-only schema of views
for `JOIN`/`UNION` queries:
...
This is the S3-hosted analogue of `ProjectCatalog.open_registries()` (which
`ATTACH`es local `.duckdb` files) -- see
[Sharing Registries via S3](tutorials/project-organization.qmd#sharing-registries-via-s3)
...
|-------|--------------|------------|
| **Schema** | `run_hash` connects configs to data; `run_id` ties data to a specific execution; use `list_export_variables()` before querying | [Analysis Tutorial](tutorials/analysis.qmd) |
