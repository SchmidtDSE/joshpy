...
Three scopes are validated -- each checks its target actually exists, and
each is documented against exactly what it joins:

| Scope | Set with | Joins against |
|-------|----------|----------------|
| `run_hash` | `tag_by_run_hash(run_hash, **tags)` | `job_configs.run_hash`, `config_parameters.run_hash`, `job_runs.run_hash`, `cell_data.run_hash` (same value, denormalized on purpose) |
...
registry.get_tags_by_run_hash(run_hash)
# {'site': 'JOTR001', 'biome': 'desert'}
```

`DiagnosticQueries.get_parameter_comparison(variable, param_name)` falls back
to `run_hash`-scoped tags for any `param_name` that isn't a declared sweep
parameter, so `group_by="site"` works with no extra query code once tagged.
Reaching for the join directly:

```sql
SELECT
    json_extract_string(rt.tags, '$.site') AS site,
    cd.step,
    AVG(cd.averageHeight) AS mean_height
FROM cell_data cd
JOIN run_tags rt ON rt.key = cd.run_hash AND rt.scope = 'run_hash'
GROUP BY site, cd.step
```

A tag value like `"JOTR001"` above is shared by many run_hashes -- it isn't a
column anywhere in the schema by itself. For metadata about the site *itself*
(not a specific run), use a custom scope, and `find_tagged()` for the reverse
lookup (tag value → every key that carries it):

```python
registry.tag_custom("JOTR001", scope="site", n_plots=12, elevation_m=850)
registry.find_tagged("site", "JOTR001")
# ['hash_jotr001', 'hash_jotr002', ...]
```

`tag_custom()` refuses the three validated scope names -- use the matching
`tag_by_*` method for those. See
