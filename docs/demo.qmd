...
# Expand config into individual jobs first so we can show totals
expander = JobExpander()
job_set = expander.expand(config)

# Create a session to track this experiment
# (total_jobs/total_replicates are now computed from JobSet)
...
# Jobs already expanded above, show count
...
print(f"\n\nCompleted: {succeeded}/{len(results)} succeeded")
...
`registry.list_export_variables()`, and `registry.list_config_parameters()` to see what
data is available.
