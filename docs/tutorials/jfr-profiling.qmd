...
`diagnose_jfr()` parses the recording into sections — CPU, memory, GC,
file I/O, and thread contention — with a diagnosis of the likely bottleneck:
...
For filing GitHub issues, `summarize_jfr()` produces the raw JFR event table
using the JDK's bundled `jfr` CLI tool:
