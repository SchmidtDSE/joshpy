...
Most users should start with [Single Run & Iteration](manual-workflow.qmd)
or [SweepManager Workflow](sweep-manager.qmd). This tutorial covers the
lower-level components for users who need direct control over job expansion,
registry registration, and execution loops.
:::
...
We recommend creating a separate registry file for each experiment. Reusing
registries across experiments can cause hash collisions and query ambiguity.
See [Best Practices: Registry Lifecycle](../best-practices.qmd#registry-lifecycle) for details.
:::
...
# Create CLI (auto-downloads JAR if needed)
...
- [Single Run & Iteration](manual-workflow.qmd) - The recommended starting workflow
