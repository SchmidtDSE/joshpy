...
config = JobConfig(simulation="JoshuaTreeSim")
session_id = registry.create_session(config=config, experiment_name="jotr_sites")

for run_hash in ("hash_jotr001", "hash_jotr002"):
    registry.register_run(
        session_id, run_hash, "/tmp/sim.josh",
        "maxGrowth = 50 meters", None, {"maxGrowth": 50},
    )

registry.tag_by_run_hash("hash_jotr001", site="JOTR001", biome="desert")
registry.tag_by_run_hash("hash_jotr002", site="JOTR002", biome="desert")

print(registry.get_tags_by_run_hash("hash_jotr001"))
```
...
-- no extra query code needed:

```{python}
#| label: tags-group-by
run_id_1 = registry.start_run("hash_jotr001", session_id=session_id)
registry.complete_run(run_id_1, exit_code=0)
run_id_2 = registry.start_run("hash_jotr002", session_id=session_id)
registry.complete_run(run_id_2, exit_code=0)
...
registry.conn.execute(
    ...
    "VALUES (?, 'hash_jotr001', 0, 0, 4.2)", [run_id_1],
)
registry.conn.execute(
    "INSERT INTO cell_data (run_id, run_hash, step, replicate, averageHeight) "
    "VALUES (?, 'hash_jotr002', 0, 0, 5.1)", [run_id_2],
)

from joshpy.cell_data import DiagnosticQueries
diag = DiagnosticQueries(registry)
print(diag.get_parameter_comparison("averageHeight", "site"))
```
