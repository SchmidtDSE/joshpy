...
2. **Model templating** (`.josh.j2`) to avoid duplicating model files
3. **Config templating** (`.jshc.j2`) vs raw config files (`.jshc`)
4. **Data directory helpers** to reduce file-mapping boilerplate
5. **Cross-experiment tracking** with `ProjectCatalog`
...
├── grid_configs.py                     # Grid template_vars definitions
└── notebooks/
    ├── run.ipynb
    └── analysis.ipynb
```
...
- **`data/grids/<grid_name>/`** -- All `.jshd` files for a grid in one directory. `discover_jshd_files()` reads from here.
- **`experiments/`** -- One `.duckdb` registry per experiment. Named descriptively.
- **`grid_configs.py`** -- Plain Python dicts, no joshpy imports needed.

## Grid Configurations

Grid-specific values (bounds, resolution, export paths, debug flags) are plain
dicts that get passed as `template_vars`:
...
GRIDS = {
    "dev_fine": {
        ...
        "grid_size": 30,
        "grid_low_lat": 33.902, "grid_low_lon": -116.0465,
        "grid_high_lat": 33.908, "grid_high_lon": -116.0395,
        "steps_high": 86,
        "export_prefix": "canonical_dev_fine",
        "debug": True,
    },
    "test_fine": {
        "simulation_name": "CanonicalTestFine",
        "grid_size": 30,
        "grid_low_lat": 33.5, "grid_low_lon": -116.4,
        "grid_high_lat": 34.0, "grid_high_lon": -115.4,
        "steps_high": 86,
        "export_prefix": "canonical_test_fine",
        "debug": False,
    },
}
```

These dicts have no dependency on joshpy. They're just data.

## Model Templating (.josh.j2)

### The problem

When the same model logic runs across multiple grid configurations, maintaining
separate `.josh` files (one per grid) is error-prone. Any model logic change
must be manually replicated across all copies. In practice this means copy-paste
and subtle sync bugs.

### The solution

Use `source_template_path` to template the `.josh` file with Jinja2, just like
`.jshc.j2` config templates. The template is rendered **once** with
