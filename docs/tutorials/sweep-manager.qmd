...
If jobs fail, check `result.stderr` for Josh error messages. Common issues include
unit mismatches, missing config files, and undefined identifiers. See 
[Best Practices: Error Diagnosis](../best-practices.qmd#error-diagnosis) 
for a guide to interpreting error messages.
:::
...
SweepManager also supports single runs (no sweep) using raw `.jshc` files, and
`.josh.j2` model templating to avoid duplicating model files across grid
configurations. See the **[Project Organization](project-organization.qmd)**
tutorial for the full guide, including:

- `config_path` for raw `.jshc` files (no Jinja rendering)
- `source_template_path` for `.josh.j2` model templates
- `discover_jshd_files()` for auto-building file mappings
- Recommended directory layout for multi-grid projects
...
- [Sweeping Over External Data Files](external-data-sweep.qmd) - FileSweepParameter for data file sweeps
