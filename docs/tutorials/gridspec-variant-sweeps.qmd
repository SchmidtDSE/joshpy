...
`describe()` prints a full overview -- geometry, variant axes, and the
external-data inventory, including whether each `.jshd` file exists on disk. It's
the quickest way to confirm what a preprocessed grid actually provides:
...
The same overview is available from the terminal without writing any Python:

```bash
python -m joshpy.grid examples/external_data/variant_sweep_grid.yaml

# structured JSON (for scripting)
python -m joshpy.grid examples/external_data/variant_sweep_grid.yaml --json
```

`to_summary_dict()` returns that structured form programmatically. The sections
below break the same information into the individual accessors.
