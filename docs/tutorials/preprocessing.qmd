...
For a more comprehensive check, use `inspect_jshd()` and `plot_jshd()` to dump
...
for ax, pattern in zip(axes, patterns):
    ...
    # Load JSHD data
    data = inspect_jshd(cli, jshd_path)
    arr = data.to_array(timestep=0)
    
    # Plot
    im = ax.imshow(arr, origin="lower", cmap="YlGn", aspect="auto")
...
data = inspect_jshd(cli, DATA_DIR / "soil_quality_gradient.jshd")
...
4. **Visualized the full grid** using `inspect_jshd()` and `plot_jshd()` to confirm patterns
...
- [JSHD Inspection](../reference/jshd.qmd) - `inspect_jshd()` and `plot_jshd()` reference
