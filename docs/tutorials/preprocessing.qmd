...
| `time_type` | Declared temporal axis type: `"count"` or `"ISO"` (optional) |
| `time_start` | First count coordinate or ISO date (optional) |
| `time_unit` | Unit for a count axis, such as `"year"` (optional) |
| `time_count` | Number of declared temporal coordinates (optional) |
| `time_increment` | Increment between count coordinates (optional; JAR default: `1`) |
| `time_interval` | ISO-8601 period between ISO dates, such as `"P1M"` (optional) |
| `time_instant` | Single count coordinate or ISO date for a one-slice output (optional) |
| `amend` | Add to existing `.jshd` file (default: False) |
| `crs` | Coordinate reference system |
| `parallel` | Enable parallel processing (~Nx speedup on N cores) |

### Declared temporal axes

By default, preprocessing preserves the source's raw slice layout. Set temporal
axis options when the output should carry explicit coordinates for Josh's
coordinate-based external reads.

For a count axis, declare the first coordinate, its unit, the number of output
slices, and optionally its increment:
...
result = cli.preprocess(NetcdfPreprocessConfig(
    ...
    variable="soil_quality",
    units="percent",
    output=DATA_DIR / "soil_quality_annual.jshd",
    time_type="count",
    time_start=2015,
    time_unit="year",
    time_count=86,
...
For ISO dates, use an ISO-8601 period rather than a count unit and increment:
...
result = cli.preprocess(NetcdfPreprocessConfig(
    ...
    variable="soil_quality",
    units="percent",
    output=DATA_DIR / "soil_quality_monthly.jshd",
    time_type="ISO",
...
Use `time_instant` instead of a range to declare a one-slice count coordinate
or ISO date. The Josh JAR validates incompatible combinations and verifies that
the declared count matches the output slices.
...
Steps 2--6 showed the low-level `cli.preprocess()` API where you manage output
paths and file inventories yourself. **GridSpec** provides a higher-level
workflow: you preprocess *through* the grid, and it automatically registers each
file and writes a `grid.yaml` manifest. This is the recommended approach for
structured projects.

### Static files (auto-registered)

When you call `grid.preprocess_netcdf()` without a `variant` argument, the
output goes to `output_dir/{josh_name}.jshd` and the file is auto-registered
in `grid.files`:

```{python}
#| label: gridspec-static
import tempfile
from pathlib import Path
...
The resulting `grid.yaml` contains both grid geometry and the file inventory.
Load it later with `GridSpec.from_yaml()` to get `file_mappings` and
`template_vars` for `JobConfig`.

### Temporal grids

For time-varying resources, declare the shared axis on the GridSpec rather
than repeating temporal options for every NetCDF file:

```python
from joshpy.grid import TimeAxis

grid = GridSpec(
    name="future-climate",
    output_dir=Path("data/grids/future-climate"),
    size_m=1000,
    low=(34.0, -116.4),
    high=(33.7, -115.4),
    time=TimeAxis(
...
grid.preprocess_netcdf(
    ...
    data_file=DATA_DIR / "future_temperature.nc",
    variable="tas",
    units="K",
    compress=True,
)
...
The declared axis supplies Josh's `--time-*` preprocessing arguments and
renders 86 physical slices (`steps.low = 0`, `steps.high = 85`). The saved
manifest contains the same axis under `time:`. Use a per-resource
`TimeAxis(...)` override only when a resource intentionally has a different
cadence or range.

### Variant files

When data files vary by scenario (e.g., different soil patterns or climate
projections), declare **variant axes** in the GridSpec and preprocess each
value with the `variant` parameter. This produces the `template_path` entries
that power `variant_sweep()`.

```{python}
#| label: gridspec-variant
...
### Batch dispatch (remote)

Each `preprocess_*()` method above has a `preprocess_*_batch()` counterpart --
`preprocess_geotiff_batch()`, `preprocess_netcdf_batch()`,
`preprocess_csv_batch()` -- for offloading preprocessing to a remote target
(Kubernetes or HTTP) instead of running it locally. They're identical in every
other respect: same output-path/variant/`template_path` resolution, same
declared-temporal-axis handling and stub-script sizing, same `grid.files`
registration on success. The only differences are a required `target` (a
target profile name, same as `JoshCLI.preprocess_batch()`) and optional
`poll_interval`/`timeout` for how long to wait for the remote job:

```python
# Same call as the "Temporal grids" example above, dispatched remotely
...
This blocks until the remote job completes (or fails) and registers
`grid.files["futureTemp"]` on success, exactly like the local call -- no
reaching into `GridSpec`'s internals to reproduce output-path or registration
logic for the remote path. See [Remote Execution](remote-execution.qmd) for
target profile setup.

### Loading back

Once saved, the YAML is the single source of truth for grid geometry and data
inventory:
...
| `grid.preprocess_*_batch()` | Same as above, offloaded to a remote target instead of run locally |

All three produce identical `.jshd` files -- the differences are whether the
inventory is tracked in a YAML manifest or managed manually, and whether
preprocessing runs locally or on a remote target.
:::
...
7. **Offloaded preprocessing to a remote target** using `grid.preprocess_netcdf_batch()`
   in place of the local call, with identical output-path and manifest semantics
...
how to run parameter sweeps over multiple external data files. For the
GridSpec-based variant sweep workflow, see
...
- [GridSpec Variant Sweeps](gridspec-variant-sweeps.qmd) - Use GridSpec variants in parameter sweeps
- [External Data Sweep Tutorial](external-data-sweep.qmd) - Use these files in a file sweep
- [Project Organization](project-organization.qmd) - Directory layout and GridSpec conventions
