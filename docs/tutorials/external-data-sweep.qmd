...
In ecological modeling, you often need to run the same simulation with **different
input data** - for example, comparing climate scenarios (RCP 2.6 vs 4.5 vs 8.5) or
testing sensitivity to soil maps from different sources.
...
For custom analysis, query the registry directly:

```{python}
#| label: sql-analysis
# Compare mean height by soil pattern
# File parameters are stored as JSON in job_configs.file_mappings
# We extract the filename to identify the pattern
result = manager.registry.query(r"""
    SELECT 
        regexp_extract(jc.file_mappings, 'soil_quality_([a-z]+)\.jshd', 1) as pattern,
        cd.step,
        AVG(cd.average_height) as mean_height,
        STDDEV(cd.average_height) as std_height
    FROM cell_data cd
    JOIN job_configs jc ON cd.run_hash = jc.run_hash
    GROUP BY pattern, cd.step
    ORDER BY pattern, cd.step
""")
result.df().head(15)
```
...
| **Storage** | `job.parameters` dict | `job.file_mappings` dict |
| **Josh reference** | `config name.variable` | `external variable` |
| **CLI flag** | `--custom-tag` | `--data` |
