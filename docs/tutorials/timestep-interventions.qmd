...
```josh
# Read once at patch creation (static flat-time data → pin to record 0)
initial_density.init = external initial_density at index 0

# Use to create variable populations (density/5 gives 0-20 trees)
ForeverTree.init = create round(initial_density / 5 percent) of ForeverTree
```

**Pattern 2: Conditional timestep events**
```josh
# Read severity at init (flat-time data → pin to record 0), persist through steps
fire_severity.init = external fire_severity at index 0
...
|---------|-------|-------------|
| **Init-time** | `*.init` | `density.init = external initial_density at index 0` |
