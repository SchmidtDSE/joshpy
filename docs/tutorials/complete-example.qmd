...
Both runs share the same registry, so comparison is one line:
...
To see exactly what changed between the two configs:

```{python}
import tempfile

tmpdir = tempfile.mkdtemp()
p1 = registry.export_config("gradient_low", tmpdir)
p2 = registry.export_config("stripes_high", tmpdir)
...
The config diff shows parameter changes, but what about external data files?
`get_file_mappings()` returns the `.jshd` paths that were used for each run:
...
```bash
python -m joshpy.config complete_demo.duckdb --diff gradient_low stripes_high
```

See `python -m joshpy.config --help` for IDE selection and export-only mode.
:::
