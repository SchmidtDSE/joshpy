...
`"gradient_low"`? Use `on_collision="supersede"` to retire the old run — it
releases the label and is marked superseded — so the bare label always points
to your latest attempt:
...
manager = (
    ...
    .with_cli(cli)
    .with_label("gradient_low", on_collision="supersede", reason="tweaked maxGrowth")
    .with_catalog(catalog, experiment_name="adhoc_iteration")
    .build()
)
...
The old `"gradient_low"` run is now superseded — it released its label, and the
bare label points to the new run. The superseded run stays in the registry for
provenance (walk it with `run_history("gradient_low")`) but no longer appears in
`list_labels()`:

```{python}
registry = RunRegistry(ADHOC_REGISTRY)
print("Labels after supersession:")
...
Both current labeled runs (the latest `gradient_low` and `stripes_high`) appear
in the comparison; the superseded run dropped its label and is excluded:
