...
### [Inspecting Runs from the Registry](inspecting-runs.qmd)
...
### [Bottling Runs for Reproducibility](bottling-runs.qmd)
...
### [Finding Bottlenecks](jfr-profiling.qmd)
...
### [Spin-up / Spin-down](spinup-spindown.qmd)

Warm a simulation to equilibrium before the observed period and watch recovery
after:

- Multiple named simulations (plain vs. warmed-up) in one `.josh` file
- The phase-anchored clock (negative spin-up steps, `step 0` = first observed)
- `output_phases` to control which phases are exported

Best for: Models that need to reach equilibrium before the observed data window.
